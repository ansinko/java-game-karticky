...
public class GameState implements JSONSerializable {
    ...
    private boolean canStepFrom(TilePos origin) {
        // players are placing their troops
        if (!orangeArmy.boardTroops().isLeaderPlaced() || orangeArmy.boardTroops().isPlacingGuards()
                || !blueArmy.boardTroops().isLeaderPlaced() || blueArmy.boardTroops().isPlacingGuards())
            return false;
        // the game is not in play or the tile is empty or filled with the troop of another player
    ...
    private boolean canStepTo(TilePos target) {
        ...
        return result == GameResult.IN_PLAY && armyOnTurn().boardTroops().at(target).isEmpty()
                && board.at(target).canStepOn() && armyNotOnTurn().boardTroops().at(target).isEmpty();
    }
